...
### ⚠️ Prerequisite for flows 8 + 9: the impl gap

`LaunchSequenceServiceImpl` + `RunScheduleServiceImpl` don't exist yet (see [7.2](./7.2-unit-domain-services.md)). Those impls must land before flows 8 + 9 can be written. If you're doing 7.7 before 7.2's impl work is done, write the other 10 flows first and circle back.
